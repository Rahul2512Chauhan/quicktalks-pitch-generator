...
def generate_all_outputs(title:str , description:str , keywords:str = "") -> dict:
    ...
    #Get prompts
    prompts_30 = get_30s_pitch_prompt(title , description , keywords)
    prompts_60 = get_60s_pitch_prompt(title , description , keywords)
    prompts_resume = get_resume_bulles_prompts(title , description , keywords)

    #Get model outputs
    pitch_30 = query_zephyr_locally(prompts_30)
    pitch_60 = query_zephyr_locally(prompts_60)
    resume_bullets_raw = query_zephyr_locally(prompts_resume)
